...
async function renderPersonMap(el, p) {
  ...
  if (!fullAddress) {
    el.innerHTML = `<div class="empty-state"><div class="empty-text">Kein Standort für diese Person hinterlegt.</div></div>`;
    return;
  }
  
  el.innerHTML = `
    <div class="section-header">
      <span class="section-title">Standort: ${fullAddress}</span>
    ...
    <div class="map-container" id="person-map" style="height: calc(100vh - 380px); min-height: 400px; border-radius: 12px; overflow: hidden;"></div>
  `;
  if (window.lucide) window.lucide.createIcons();
  
  // Dynamic import of map logic
  const { renderMap } = await import('./map.js');
  // Initialize a smaller map centered on the person
  setTimeout(async () => {
    ...
    if (lat === undefined || lng === undefined) {
      // Small geocoding helper here or just rely on map.js logic
      // For simplicity, we'll use a simplified version of map.js logic
    ...
    if (lat !== undefined && lng !== undefined) {
      const pMap = L.map('person-map', { attributionControl: false }).setView([lat, lng], 13);
      ...
      const icon = L.divIcon({
        ...
        html: `<div class="map-marker-avatar ${riskClass(p.riskLevel)}" style="width:40px; height:40px;">${avatarEl(p, 40)}</div>`,
        iconSize: [40, 40],
        iconAnchor: [20, 20]
      });
      L.marker([lat, lng], { icon }).addTo(pMap);
    } else {
      el.innerHTML += `<div style="padding:20px; color:var(--red); font-family:var(--font-mono);">Standort "${fullAddress}" konnte nicht geocodiert werden.</div>`;
    }
  }, 100);
}
