...
async function renderDashboard() {
  ...
  const totalDocs = await import('./db.js').then(db => db.Files.listAll().then(f => f.length));
...
async function renderSettings() {
  ...
  document.getElementById('import-file').onchange = async (e) => {
    ...
    } catch(e) { showToast('Import fehlgeschlagen: ' + e.message, 'red'); }
  };
...
async function renderArchive() {
  setHeader('Archiv', 'archive');
  const [persons, interactions, files] = await Promise.all([
    Persons.list(), Interactions.listAll(),
    import('./db.js').then(db => db.Files.listAll()),
  ]);
  workspace().innerHTML = `
    <div class="dash-grid mb-4">
      <div class="dash-stat stat-cyan"><div class="dash-stat-label">Personen</div><div class="dash-stat-value">${persons.length}</div></div>
      <div class="dash-stat stat-gold"><div class="dash-stat-label">Interaktionen</div><div class="dash-stat-value">${interactions.length}</div></div>
      <div class="dash-stat" style=""><div class="dash-stat-label">Dateien</div><div class="dash-stat-value" style="color:var(--purple);">${files.length}</div></div>
    </div>

    <div class="section-header mb-3"><span class="section-title">Alle Interaktionen</span></div>
    <div style="max-height:400px;overflow-y:auto;">
      ${interactions.sort((a,b) => new Date(b.date)-new Date(a.date)).slice(0,30).map(i => {
        const p = persons.find(x => x.id === i.personId);
        return `<div class="card mb-2" style="cursor:pointer;" onclick="navigate('person/${i.personId}')">
          <div style="font-size:9px;color:var(--text-muted);font-family:var(--font-mono);">${new Date(i.date).toLocaleString('de-DE')} · ${i.type} · ${p?.name||'–'}</div>
          <div style="font-size:12px;margin-top:4px;">${i.content.slice(0,200)}</div>
        </div>`;
      }).join('') || '<div class="empty-state"><div class="empty-text">Keine Einträge</div></div>'}
    </div>
  `;
}
