  ...
  const render = async () => {
    ...
    container.innerHTML = `
      <div class="section-header mb-4">
        ...
        <div class="flex gap-2 person-filters">
          <button class="btn btn-primary btn-sm" id="btn-add-person">＋ <span class="hide-on-mobile">Neue Person</span></button>
          ...
          <input class="form-input" id="filter-search" placeholder="Suchen..." value="${filterText}">
        </div>
      </div>
      ${filtered.length === 0 ? `
...
export function openPersonModal(person = null, onSave) {
      ...
      <div class="modal-body">
        <div class="grid-2">
          <div>
            ...
            <div class="form-group">
              <label class="form-label">Standort (Stadt / Land)</label>
              <input class="form-input" id="p-location" value="${p.location || ''}" placeholder="z.B. Berlin, Deutschland">
            </div>
            <div class="grid-2">
              <div class="form-group">
                <label class="form-label">Straße</label>
                <input class="form-input" id="p-street" value="${p.street || ''}" placeholder="Hauptstr.">
              </div>
              <div class="form-group">
                <label class="form-label">Nr.</label>
                <input class="form-input" id="p-house-number" value="${p.houseNumber || ''}" placeholder="12a">
              </div>
            </div>
            <div class="form-group">
  ...
  overlay.querySelector('#modal-save').onclick = async () => {
    ...
    const obj = {
      ...
      street: overlay.querySelector('#p-street').value,
      houseNumber: overlay.querySelector('#p-house-number').value,
      createdAt: p.createdAt || new Date().toISOString(),
    };
    
    // If address info changed, clear cached coordinates to trigger re-geocoding
    if (obj.location !== p.location || obj.street !== p.street || obj.houseNumber !== p.houseNumber) {
      delete obj.lat;
      delete obj.lng;
    }
